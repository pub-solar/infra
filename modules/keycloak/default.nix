...
in
{
  ...
  config = lib.mkIf config.pub-solar-os.auth.enable {
    ...
    services.keycloak = {
      ...
      themes = {
        "pub.solar" =
          flake.inputs.keycloak-theme-pub-solar.legacyPackages.${pkgs.stdenv.hostPlatform.system}.keycloak-theme-pub-solar;
      };
      plugins = [ flake.inputs.keycloak-event-listener.packages.${pkgs.stdenv.hostPlatform.system}.keycloak-event-listener ];
    };

    pub-solar-os.backups = {
      resources.keycloak-db.resourceCreateCommand = ''
        ${pkgs.sudo}/bin/sudo -u postgres ${config.services.postgresql.package}/bin/pg_dump -d keycloak | ${pkgs.zstd}/bin/zstd --force --quiet -o /tmp/keycloak-backup.sql
      '';
      restic.keycloak = {
